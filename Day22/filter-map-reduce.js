// const myArr = [10, 20, 30, 40, 50];

// const values = myArr.forEach( (item) => {
//     console.log(item);
// })
// console.log(values); // doesn't return the value

/*
const values1 = myArr.filter((num) => num > 40);
 
const values2 = myArr.filter((num) => {
   return num > 40; // return must be used while using parenthesis i.e defining scope
});

console.log(values1);
console.log(values2);
*/
/*
const newArr = [];
myArr.forEach ( (num) => {
if (num > 40) {
    newArr.push(num);
}
});
console.log(newArr);
*/

/* exercise 

const books = [
    { title: 'Book One', genre: 'Fiction', publish: 1981, edition: 2004 },
    { title: 'Book Two', genre: 'Non-Fiction', publish: 1992, edition: 2008 },
    { title: 'Book Three', genre: 'History', publish: 1999, edition: 2007 },
    { title: 'Book Four', genre: 'Non-Fiction', publish: 1989, edition: 2010 },
    { title: 'Book Five', genre: 'Science', publish: 2009, edition: 2014 },
    { title: 'Book Six', genre: 'Fiction', publish: 1987, edition: 2010 },
    { title: 'Book Seven', genre: 'History', publish: 1986, edition: 1996 },
    { title: 'Book Eight', genre: 'Science', publish: 2011, edition: 2016 },
    { title: 'Book Nine', genre: 'Non-Fiction', publish: 1981, edition: 1989 },
  ];

const userBooks1 = books.filter( (bk)=>{
return bk.genre === "History"
});
const userBooks2 = books.filter( (bk) => {
  return (bk.publish >= 1980 && bk.genre === "History");
});
console.log(userBooks1);
console.log(userBooks2);
*/

/* Map */
// const myArr = [10, 20, 30, 40, 50];

// const newNums = myArr.map( (num) => {
// return num + 10;
// });

/* chaining
const newNums = myArr.map( (num)=>{
  return num * 10;
})
.map( (num) => {
  return num + 10;
})
.filter( (num) => {
  return num>=300;
})

console.log(newNums);
*/

/* Reduce */
/*
const myArr = [10, 20, 30, 40, 50];

// const myTotal = myArr.reduce( function (acc, currval) {
//   console.log(`acc: ${acc} & curval: ${currval}`);
//   return acc + currval;
// }, 3); // initial value after , is 0; can be changed

const myTotal = myArr.reduce ( (acc, currval)=>{
return acc + currval;
}, 0);

console.log(myTotal);
*/

const myCart = [
  {
    itemName: "js",
    price: 199
  },
  {
    itemName: "python",
    price: 599
  },
  {
    itemName: "data science",
    price: 1099
  }
]

const myTot = myCart.reduce ((acc, item) =>
{
return acc + item.price;
}, 0);

console.log(`Price to Pay : ${myTot}`);